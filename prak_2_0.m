...
%omega0 =  1.2383e-3;
%omega0 = omega0^2; %
e = 6.69437999013e-3; % excentricitet
a = 6378137; % bolsh poluos ellipsa
u = 7292115.8553e-11; % uglovaya skorost zemli
R = 6370000;

data = load('start.dat'); %trajectory
phi = data(1)/180*pi;
...
Nvi = 1;   % Nachalo vistavki
Nvo = 12000;  % Konez
Ni = 12000;   % Nachalo dvizh
...
z2 = data(:,6);
z3 = data(:,7);
fz = mean(data(1:Nvo,5:7)) %nachaln vistavka
...
long = deg2rad(data(:,3));
h = data(:,4);
Ve = data(:,5); % skorost  Ve - v1, Vn - v2
Vn = data(:,6);
Vh = data(:,7);
% Krylov angles
head=deg2rad(data(:,8));  % ryskanye
pitch=deg2rad(data(:,9));    % tangazh
roll=deg2rad(data(:,10)); % kren
numStrings = length(t2);
...
% L(:,1)=cross(L(:,2),L(:,3));
%PODSTAVLYAEM NACHALNITE UGLI IZ TRJ.DAT
L( :,3) = [-cos(teta3) * sin(gamma3); sin(teta3); cos(teta3)*cos(gamma3)];
L(:, 2) = [sin(psi3)*cos(gamma3) + cos(psi3)*sin(teta3)*sin(gamma3); cos(psi3)*cos(teta3); sin(psi3)*sin(gamma3) - cos(psi3)*sin(teta3)*cos(gamma3)];
L(:, 1) = [cos(psi3)*cos(gamma3) - sin(psi3)*sin(teta3)*sin(gamma3); -sin(psi3)*cos(teta3); cos(psi3)*sin(gamma3) + sin(psi3)*sin(teta3)*cos(gamma3)];
...
% Integrirovanie 2 matrichnykh uravneniy

Az=L; % Nachaln. usloviya Az(0) = L(0), Ax(0) = E

Ax=eye(3);

% ustanovka nach usloviy (snachala stoim na meste)
for i = 1:Ni
    lambda(i) = ld0;
    phi(i) = phi0;
    hs(i) = h0;
...
for k = Ni:(size(t)-1)
    ...
    % Coefficienty formuly (3.12)
    c1 = sin(omega*delta)/omega;
    c2 = (1-cos(omega*delta))/(omega^2) ;
    
    
    Omega = [0 , omega3(k), - omega2(k);...  %(3.4) Dlya Az
        -omega3(k),  0 , omega1(k);...
        omega2(k), - omega1(k) , 0];
    
    Az = (eye(3) + c1*Omega+ c2* Omega^2)*Az; % A_i+1  Iteracionnoe resheniye (3.12)
    ...
    % Formula (3.6). Omega - vector uglovoy skor-ti geograph trekhgrannika otnositelno
    % Zemli
    omegaE = -Vns(k)/(Rn+hs(k));  % Omeg1
    omegaN = Ves(k)/(Re+hs(k));   % Omeg2
    omegaH = omegaN*tan(phi(k));  %Omeg3
    
    % Novaya omega (chtob pereschitat' Ax)
    ...
    % detL = det(L)% orientaciya pribornogo 3-grannika otnositelno geograph (posle 3.7)
    fz = [z1(k); z2(k); z3(k)];
    % f (uskor/sily) v sisteme coord (z1, z2, z3)
    % perehod k Ox (???)
    fy = L'*fz;  % perehod v geograph coordinaty; fy - sily/uskoreniya otnositelno Zemli
    ...
    %6
    % sled shag. Metod Eulera
    Ves(k+1)=  Ves(k) + delta*dotVe;  % V1
    Vns(k+1)=  Vns(k) + delta*dotVn;  % V2
    Vhs(k+1)=  Vhs(k) + delta*dotVh;
    
    %7
    % Formula (3.15). Metod Eulera/ Coord samoleta v geographicheskoy sisteme
...
x1 = zeros(numStrings,1);
y1 = zeros(numStrings,1);
h1 = zeros(numStrings,1);
...
r2 = [(R + h(numStrings)) * sin(lat(numStrings)) * cos(long(numStrings)),
    (R + h(numStrings)) * sin(lat(numStrings)) * sin(long(numStrings)),
    (R + h(numStrings)) * cos(lat(numStrings))];

r = norm(r1 - r2)


deltaV1 = zeros(numStrings, 1);
deltaV2 = zeros(numStrings, 1);
deltaV3 = zeros(numStrings, 1);
...
figure(8);clf;
plot(t2(3:numStrings),deltaDotV3);
hold on;
grid on;

figure(9);clf;
plot(t2(3:numStrings),deltaDotV2);
hold on;
grid on;

figure(10);clf;
plot(t2(3:numStrings),deltaDotV1);
hold on;
grid on;
